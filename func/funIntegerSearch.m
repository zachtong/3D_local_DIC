...
switch tempNoOfInitPt % 0- whole field; 1- several seeds points;
    case 0 % 0- whole field;
        ...
        % Revised by Zach  [left right up down]
        tempSizeOfSearchRegion = repmat(tempSizeOfSearchRegion,[1,2]);
        ...
        % if gridx(1)-tempSizeOfSearchRegion(1)-0.5*winsize(1) < 1
        %     tempSizeOfSearchRegion(1) = gridx(1) - 1;
        % end
        % if gridy(1)-tempSizeOfSearchRegion(3)-0.5*winsize(2) < 1
        %     tempSizeOfSearchRegion(3) = gridy(1) - 1;
        % end
        % if gridx(end)+0.5*winsize(1)+tempSizeOfSearchRegion(2) > size(f,1)
        %     tempSizeOfSearchRegion(2) = size(f,1) - gridx(end) - 0.5 * winsize(1) - 1;
        % end
        % if gridy(end)+0.5*winsize(2)+tempSizeOfSearchRegion(4) > size(f,2)
        %     tempSizeOfSearchRegion(4) = size(f,2) - gridy(end) - 0.5 * winsize(2) - 1;
        % end
