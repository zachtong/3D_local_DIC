function [RD] = TemporalMatch_inc(DICpara,file_name,Img,ImgNormalized,RD,Df,camera0OrNot)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Extend the corresponding ROI shape in right series to rectangle
% Output the extended ROI and true subset coordinates in right camera
if strcmp(camera0OrNot,'notCamera0')
    [DICpara,RD] = extendROI(DICpara,RD);
else
end
...
for ImgSeqNum = 2 : length(ImgNormalized)
    ...
    fNormalized = Img{ImgSeqNum-1}; % Load previous reference image frame
    ...
    % ====== Integer Search ======
    [DICpara,x0temp,y0temp,u,v,cc]= IntegerSearch(fNormalized,gNormalized,file_name,DICpara,showImgOrNot);
    ...
    % Acc mode: to save first mesh info; Inc mode: save every mesh info
    RD.ResultFEMesh{1+floor(fNormalizedNewIndex/DICpara.ImgSeqIncUnit)} = ...
    ...
    [USubpb1,FSubpb1,HtempPar,ALSub1Timetemp,ConvItPerEletemp,LocalICGNBadPtNumtemp] = ...
        LocalICGN(U0,DICmesh.coordinatesFEM,Df{ImgSeqNum-1,1},fNormalized,gNormalized,DICpara,'GaussNewton',tol,1);
    ...
    % Plotdisp_show(USubpb1World,DICmesh.coordinatesFEMWorld,DICmesh.elementsFEM,[],'NoEdge50Color');
    ...
    if UseGlobal
        ...
        while (ALSolveStep < 3)
            ...
            tic;[USubpb1,~,ALSub1Timetemp,ConvItPerEletemp,LocalICGNBadPtNumtemp] = Subpb1( ...
                ...
                Df{ImgSeqNum-1,1},fNormalized,gNormalized,mu,beta,HPar,ALSolveStep,DICpara,'GaussNewton',tol);
            FSubpb1 = FSubpb2; toc
    ...
    % interpolation due to Reference image's update
    if ImgSeqNum == 2
        RD.ResultDisp_inc{1,1} = reshape(RD.ResultDisp{1}.U,[2,DICmesh.M*DICmesh.N])';
        RD.CoorEachFrame{1,1} = RD.ResultFEMesh{1, 1}.coordinatesFEM + RD.ResultDisp_inc{1,1};
    else
        % case 1
        tempU = reshape(RD.ResultDisp{ImgSeqNum-1}.U',[2,DICmesh.M*DICmesh.N])';
        % interpolation coefficient
        % calculate coefficient every frame
        rbfInterpX = rbfcreate((RD.ResultFEMesh{ImgSeqNum-1,1}.coordinatesFEM)',(tempU(:,1))');
        rbfInterpY = rbfcreate((RD.ResultFEMesh{ImgSeqNum-1,1}.coordinatesFEM)',(tempU(:,2))');
        % interpolating
    ...
    % interpolation due to extention of ROI
    interp_method = 1;
    if strcmp(camera0OrNot,'notCamera0')
        switch interp_method % Different interpolation functions
            case 1
                tempU = reshape(RD.ResultDisp_inc{ImgSeqNum-1}',[2,DICmesh.M*DICmesh.N])';
                % interpolation coefficient
                % calculate coefficient every frame
                rbfInterpX = rbfcreate((RD.ResultFEMesh{1,1}.coordinatesFEM)',(tempU(:,1))');
                rbfInterpY = rbfcreate((RD.ResultFEMesh{1,1}.coordinatesFEM)',(tempU(:,2))');

                % interpolating
                temp(1,:) = rbfinterp(RD.ResultFEMesh_corr{1,1}',rbfInterpX);
                temp(2,:) = rbfinterp(RD.ResultFEMesh_corr{1,1}',rbfInterpY);
                RD.ResultFEMesh_corr{ImgSeqNum,1} =  RD.ResultFEMesh_corr{1,1} + temp';

                % tempCoor = reshape(RD.CoorEachFrame{ImgSeqNum-1},[2,DICmesh.M*DICmesh.N])';
                % % interpolation coefficient
                % % calculate coefficient every frame
                % rbfInterpX = rbfcreate((RD.ResultFEMesh{1,1}.coordinatesFEM)',(tempCoor(:,1))');
                % rbfInterpY = rbfcreate((RD.ResultFEMesh{1,1}.coordinatesFEM)',(tempCoor(:,2))');
                %
                % % interpolating
                % temp(1,:) = rbfinterp(RD.ResultFEMesh_corr{1,1}',rbfInterpX);
                % temp(2,:) = rbfinterp(RD.ResultFEMesh_corr{1,1}',rbfInterpY);
                % RD.ResultFEMesh_corr{ImgSeqNum,1} =  temp';

                % case 2 % Don't use. Need to debug.
                %     tempU = reshape(RD.ResultDisp{ImgSeqNum-1}.U(1:2:end),[DICmesh.M,DICmesh.N]);
                %     u22 = (RD.ResultFEMesh_corr(:,1) - (DICpara.gridxyROIRange.gridx(1)+DICpara.winsize/2) ) / DICpara.winstepsize;
                %     tempV = reshape(RD.ResultDisp{ImgSeqNum-1}.U(2:2:end),[DICmesh.M,DICmesh.N]);
                %     v22 = (RD.ResultFEMesh_corr(:,2) - (DICpara.gridxyROIRange.gridy(1)+DICpara.winsize/2) ) / DICpara.winstepsize;
                %
                %     RD.ResultDisp_corr{ImgSeqNum-1} = [ba_interp2(tempU, v22, u22, 'cubic'), ba_interp2(tempV, v22, u22, 'cubic')];
        end
    end
    ...
    if showImgOrNot == 1
        ...
        if DICpara.DICIncOrNot == 1
            U_inc = reshape(RD.ResultDisp_inc{ImgSeqNum-1,1}.',[],1);
            close all; Plotuv(U_inc,DICmesh.x0,DICmesh.y0World);
            Plotdisp_show(U_inc,DICmesh.coordinatesFEMWorld,DICmesh.elementsFEM);
        else
            USubpb2World = USubpb2; USubpb2World(2:2:end) = -USubpb2(2:2:end);
            FSubpb2World = FSubpb2; FSubpb2World(2:4:end) = -FSubpb2World(2:4:end); FSubpb2World(3:4:end) = -FSubpb2World(3:4:end);
            close all; Plotuv(USubpb2World,DICmesh.x0,DICmesh.y0World);
