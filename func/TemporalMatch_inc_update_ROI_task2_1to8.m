function [RD] = TemporalMatch_inc_update_ROI_task2_1to8(DICpara,file_name,ImgNormalized,RD,StereoInfo,camera0OrNot)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% For loop 
for ImgSeqNum = 2 : 9%16  % inc. mode first
    ...
    % disp_inc2 is the disp of nodes based on the ref(first) ROI
    ...
    switch camera0OrNot
        case 'camera0'
            ...
            if ImgSeqNum == 2
                RD.ResultDisp_inc2{1,1}.U = reshape(RD.ResultDisp_inc1{1}.U,2,size(RD.ResultDisp_inc1{1}.U,1)*0.5)';
...
%             % Change the disp.U format to keep same with acc. mode
%             tempUU = RD.ResultDisp{ImgSeqNum-1,1}.U';
%             RD.ResultDisp{ImgSeqNum-1,1}.U(:,2) = [];
%             RD.ResultDisp{ImgSeqNum-1,1}.U = tempUU(:);
%         end
%     case  'notCamera0'
%         for ImgSeqNum = 2 : imageNum
%             % Change the disp.U format to keep same with acc. mode
%             tempUU = RD.ResultDisp_corr{ImgSeqNum-1,1}.U';
%             RD.ResultDisp_corr{ImgSeqNum-1,1}.U(:,2) = [];
%             RD.ResultDisp_corr{ImgSeqNum-1,1}.U = tempUU(:);
%         end
% end
