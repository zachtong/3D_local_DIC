function [P]=FeatureMatch(PD,SubSize,F,G)
...
KptsInVacinity=((abs(PD(2,:)-xk1(2,:)')<=SubSize /2) +(abs(PD(1,:)-xk1(1,:)')<=SubSize/2))==2;
xk1=xk1(:,sum(KptsInVacinity,2)>=1); d1=d1(:,sum(KptsInVacinity,2)>=1);
[matches, ~] = vl_ubcmatch(d1, d2,1.25);
	xk1=xk1(1:2,matches(1,:))';	xk2=xk2(1:2,matches(2,:))';
relevantKpts=knnsearch(xk1,PD','K',20);
...
P=NaN(12,size(PD,2));
for q=1:size(PD,2) % can be changed to parfor for parallel processing
	try
		...
		P(:,q)=[a(1)*PD(1,q)+a(2)*PD(2,q)+a(3); a(1); a(2); 0; 0; 0;a(4)*PD(1,q)+a(5)*PD(2,q)+a(6); a(4); a(5); 0; 0; 0]; % Equation (31)
	end
end
