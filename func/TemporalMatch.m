function [RD] = TemporalMatch(DICpara,file_name,fNormalized,ImgNormalized,RD,Df,camera0OrNot)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for ImgSeqNum = 2 : length(ImgNormalized)
    ...
    if strcmp(camera0OrNot,'notCamera0')
        ...
        switch interp_method % Different interpolation functions
            case 1 % irregular ROI & calculated points is less than 5000*5000
                % interpolation coefficient
                % calculate coefficient every frame
                tempX = RD.ResultDisp{ImgSeqNum-1}.U(1:2:end);
                tempY = RD.ResultDisp{ImgSeqNum-1}.U(2:2:end);

                rbfInterpX = rbfcreate(RD.ResultFEMesh{1,1}.coordinatesFEM',tempX');
                rbfInterpY = rbfcreate((RD.ResultFEMesh{1,1}.coordinatesFEM)',tempY');

                % interpolating
                temp(1,:) = rbfinterp(RD.ResultFEMesh_corr{1,1}',rbfInterpX);
                temp(2,:) = rbfinterp(RD.ResultFEMesh_corr{1,1}',rbfInterpY);
                RD.ResultDisp_corr{ImgSeqNum-1}.U = temp';

                % % New methods % Bug
                % temp(:,1) = rbfsplit(RD.ResultFEMesh{1,1}.coordinatesFEM,tempU(:,1),RD.ResultFEMesh_corr{1,1},3000,20);
                % temp(:,2) = rbfsplit(RD.ResultFEMesh{1,1}.coordinatesFEM,tempU(:,2),RD.ResultFEMesh_corr{1,1},3000,20);
                % RD.ResultDisp_corr{ImgSeqNum-1} = temp;

            case 2 % Regular ROI
                tempU = reshape(RD.ResultDisp{ImgSeqNum-1}.U',[2,DICmesh.M*DICmesh.N])';
                interp_u = scatteredInterpolant(RD.ResultFEMesh{1,1}.coordinatesFEM(:,1),RD.ResultFEMesh{1,1}.coordinatesFEM(:,2),tempU(:,1),'natural');
                RD.ResultDisp_corr{ImgSeqNum-1}(:,1).U = interp_u(RD.ResultFEMesh_corr{1,1}(:,1),RD.ResultFEMesh_corr{1,1}(:,2));
                interp_v = scatteredInterpolant(RD.ResultFEMesh{1,1}.coordinatesFEM(:,1),RD.ResultFEMesh{1,1}.coordinatesFEM(:,2),tempU(:,2),'natural');
                RD.ResultDisp_corr{ImgSeqNum-1}(:,2).U = interp_v(RD.ResultFEMesh_corr{1,1}(:,1),RD.ResultFEMesh_corr{1,1}(:,2));

                % case 3 % Don't use. Need to debug.
