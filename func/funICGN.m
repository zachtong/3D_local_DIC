function [U,F,stepwithinwhile,HGlobal,varargout] = funICGN(...
...
if length(DfDxImgMaskIndRow)<0.50*(winsize+1)^2
    ...
    while( (stepwithinwhile<=100) && (normOfWNew>tol) && (normOfWNewAbs>tol) )
        ...
        if ~isempty([row1; row2; row3; row4])
            ...
            % tempg = interp2(ImgDef, v22 , u22 , 'spline');
            ...
            normOfWNew = norm(b(:));
            normOfWNewAbs = normOfWNew; % Calculate absolute value of |b|

            if stepwithinwhile ==1
                normOfWNewInit = normOfWNew;
            end
            if normOfWNewInit > tol
                normOfWNew = normOfWNew/normOfWNewInit; % Calculate relative value: |b|/|b_init|
            else
                normOfWNew = 0;
            end
