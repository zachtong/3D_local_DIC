function [x2,y2,disp_x,disp_y,...
    ...
    strain_principal_max,strain_principal_min,strain_maxshear,strain_vonMises] = Plotstrain_inc( ...
    Coor_2D,coefficients,Rad,x0,y0,sizeOfImg,CurrentImg,DICpara)
...
%% Zach revised!!
if DICpara.winstepsize > 10%4 % When step size is smaller than 4, no need to refine 
    ...
    x_reshaped = reshape (Coor_2D(:,1),M+2*Rad,N+2*Rad);
    x_withStrainData =  x_reshaped(Rad+1:end-Rad, Rad+1:end-Rad);
    y_reshaped = reshape (Coor_2D(:,2),M+2*Rad,N+2*Rad);
    y_withStrainData =  y_reshaped(Rad+1:end-Rad, Rad+1:end-Rad);

    coor_x = gridfit(reshape(x,M*N,1),reshape(y,M*N,1),reshape(x_withStrainData,M,N,1),x2,y2);
    coor_y = gridfit(reshape(x,M*N,1),reshape(y,M*N,1),reshape(y_withStrainData,M,N,1),x2,y2);
    ...
    coor_x = x_withStrainData';
    coor_y = y_withStrainData';
...
hold on; ax2=axes; h2=surf(coor_x, ...
    sizeOfImg(2)+1-coor_y, strain_exx, 'EdgeColor','none','LineStyle','none');
...
axis on; axis equal; axis tight; box on; set(gca,'fontSize',18); view(2); set(gca,'ydir','normal');
hold on; ax2=axes; h2=surf( (x2(3*crop+1:end-3*crop,crop+1:end-crop)+Image2PlotResults*disp_x(3*crop+1:end-3*crop,crop+1:end-crop)), ...
    sizeOfImg(2)+1-(y2(3*crop+1:end-3*crop,crop+1:end-crop)+Image2PlotResults*disp_y(3*crop+1:end-3*crop,crop+1:end-crop)), strain_exx(3*crop+1:end-3*crop,crop+1:end-crop), 'EdgeColor','none','LineStyle','none');
set(gca,'fontSize',18); view(2); box on;  % set(gca,'ydir','normal');
alpha(h2,OrigDICImgTransparency);  axis equal;  axis tight;  colormap(cMap);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% TODO: manually modify colormap and caxis %%%%%%
...
axis on; axis equal; axis tight; box on; set(gca,'fontSize',18); view(2); set(gca,'ydir','normal');
hold on; ax2=axes; h2=surf(coor_x, ...
    sizeOfImg(2)+1-coor_y, strain_exy,'EdgeColor','none','LineStyle','none');
...
axis on; axis equal; axis tight; box on; set(gca,'fontSize',18); view(2); set(gca,'ydir','normal');
hold on; ax2=axes; h2=surf(coor_x, ...
    sizeOfImg(2)+1-coor_y,  strain_eyy,'EdgeColor','none','LineStyle','none');
set(gca,'fontSize',18); view(2); box on;  clim auto; % set(gca,'ydir','normal');
alpha(h2,OrigDICImgTransparency);  axis equal;  axis tight;  colormap(cMap);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% TODO: manually modify colormap and caxis %%%%%%
...
if 0
    ...
    axis on; axis equal; axis tight; box on; set(gca,'fontSize',18); view(2); set(gca,'ydir','normal');
    hold on; ax2=axes; h2=surf(coor_x, ...
    sizeOfImg(2)+1-coor_y,  strain_principal_max,'EdgeColor','none','LineStyle','none');
    set(gca,'fontSize',18); view(2); box on;  clim auto; % set(gca,'ydir','normal');
    alpha(h2,OrigDICImgTransparency);  axis equal;  axis tight;  colormap(cMap);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%% TODO: manually modify colormap and caxis %%%%%%
    ...
    axis on; axis equal; axis tight; box on; set(gca,'fontSize',18); view(2); set(gca,'ydir','normal');
   hold on; ax2=axes; h2=surf(coor_x, ...
    sizeOfImg(2)+1-coor_y,  strain_principal_min,'EdgeColor','none','LineStyle','none');
    set(gca,'fontSize',18); view(2); box on;  clim auto; % set(gca,'ydir','normal');
    alpha(h2,OrigDICImgTransparency);  axis equal;  axis tight; colormap(cMap);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%% TODO: manually modify colormap and caxis %%%%%%
    ...
    axis on; axis equal; axis tight; box on; set(gca,'fontSize',18); view(2); set(gca,'ydir','normal');
    hold on; ax2=axes; h2=surf(coor_x, ...
    sizeOfImg(2)+1-coor_y,  strain_maxshear,'EdgeColor','none','LineStyle','none');
    set(gca,'fontSize',18); view(2); box on;  clim auto; % set(gca,'ydir','normal');
    alpha(h2,OrigDICImgTransparency);  axis equal;  axis tight; colormap(cMap);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%% TODO: manually modify colormap and caxis %%%%%%
    ...
    axis on; axis equal; axis tight; box on; set(gca,'fontSize',18); view(2);  set(gca,'ydir','normal');
    hold on; ax2=axes; h2=surf(coor_x, ...
    sizeOfImg(2)+1-coor_y,  strain_vonMises,'EdgeColor','none','LineStyle','none');
