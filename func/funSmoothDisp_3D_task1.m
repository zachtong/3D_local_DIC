function ULocal = funSmoothDisp_3D_task1(ULocal,DICmesh,DICpara,coordinatesFEM)
...
while (DoYouWantToSmoothOnceMore==0)
    ...
    % Iblur_Top10 = gridfit(coordinatesFEM(:,1), coordinatesFEM(:,2), ULocal{1,1},Coordxnodes,Coordynodes,'regularizer','springs'); Iblur_Top10=Iblur_Top10';
    % Iblur_Top20 = gridfit(coordinatesFEM(:,1), coordinatesFEM(:,2), ULocal{1,2},Coordxnodes,Coordynodes,'regularizer','springs'); Iblur_Top20=Iblur_Top20';
    % Iblur_Top30 = gridfit(coordinatesFEM(:,1), coordinatesFEM(:,2), ULocal{1,3},Coordxnodes,Coordynodes,'regularizer','springs'); Iblur_Top30=Iblur_Top30';
     
    % %% task1 ----------------------
    Iblur_Top10 = reshape(ULocal{1},[DICmesh.N,DICmesh.M]);
    Iblur_Top20 = reshape(ULocal{2},[DICmesh.N,DICmesh.M]);
    Iblur_Top30 = reshape(ULocal{3},[DICmesh.N,DICmesh.M]);
    %% --------------------
    ...
    for tempi = 1:size(coordinatesFEM,1)
        ...
        ULocal{1,1}(tempi) = Iblur_Top1(row1,row2);
        ULocal{1,2}(tempi) = Iblur_Top2(row1,row2);
        ULocal{1,3}(tempi) = Iblur_Top3(row1,row2);
    end
