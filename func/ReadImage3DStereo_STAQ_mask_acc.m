function [full_path_0, full_path_1, masks_0, masks_1] = ReadImage3DStereo_STAQ_mask_acc(imageLeft,imageRight)
% ==============================================
fprintf('\n Do you want to load masks?  \n')
fprintf('     0: No;  \n')
fprintf('     1: Yes;  \n')
prompt = 'Input here: ';
LoadMaskOrNot = input(prompt);
if LoadMaskOrNot == 0
    fprintf('\n Please define your ROI: \n')
        % Left mask drawing
        masks_0{1} = createMask(imageLeft); % figure; imshow(maskLeft)
        % Right mask drawing
        masks_1{1} = createMask(imageRight);
        
        full_path_0{1} = [];
        full_path_1{1} = [];
        return 

elseif LoadMaskOrNot == 1
...
[masks_original_0,masks_original_1] = convertMasksTo8bit(full_path_0,full_path_1);

% Define ROI based on masks or not?
    fprintf('\n Do you want to define ROI?  \n')
    fprintf('     0: No, just use the whole masks. \n')
    fprintf('     1: Yes, I want to define it.\n')
    prompt = 'Input here: ';
    ROIDefineOrNot = input(prompt);
    if ROIDefineOrNot == 0
        masks_0 = masks_original_0;
        masks_1 = masks_original_1;
    elseif ROIDefineOrNot == 1
        % User need to draw ROIs based on the masks they inported
        % Left mask drawing
        masks_user_defined_0 = createMask(imageLeft.*masks_original_0{1});
        % Right mask drawing
        masks_user_defined_1 = createMask(imageRight.*masks_original_1{1});
        
        for i = 1:size(masks_original_0,1)
            masks_0{i} = masks_original_0{i} .* masks_user_defined_0;
            masks_1{i} = masks_original_1{i} .* masks_user_defined_1;
        end
